...
ROOTPATH="/data/caitken/Ingolia/intermediary/RNAseq"
...
fastx_clipper -a CTGTAGGCACCATCAAT -l 25 -n -v -i /data/caitken/files/${NAME}.fq -Q 33 -o ${ROOTPATH}/${NAME}_clipped.fq &
echo "removing adaptor..."
wait

#Trimming mrnas to remove first and keep only 44 nt

#from _clipped to _trimmed
fastx_trimmer -Q 33 -i ${ROOTPATH}/${NAME}_clipped.fq -f 2 -l 45 -o ${ROOTPATH}/${NAME}_trimmed.fq &
echo "trimming mrnas"
wait
...
echo "bowtie for removing rRNAs in RNA reads"
wait
...
echo "generating alignment for  mRNAs..."
